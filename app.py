...
Original file is located at
    https://colab.research.google.com/drive/1JVO4t9wqGgWePnGjtkFvvguTxji8CzY_
"""
...
if st.button("🔄 Descargar Panel en Vivo y Calcular", type="primary"):
    ...
    with st.spinner('Conectando a Data912 y DolarAPI...'):
        ...
        for ticker, info in bonos_maestros.items():
            # Buscamos en el diccionario vivo los precios de la especie D y O
            ...
            # Ajuste de lámina y Tipo de Cambio Implícito
            ...
            try:
                ...
                mod_dur = (suma_t_pv / pv_total) / (1 + tir_usd)

                resultados.append({
                    "Ticker": ticker,
                    "Precio ARS": p_ars,
                    "Precio USD": precio_inversion_usd,
                    "TC Implícito": dolar_cable,
        ...
        if resultados:
            ...
            fig, ax = plt.subplots(figsize=(12, 6), facecolor=C_CREMA)
            ax.set_facecolor(C_CREMA)

            x, y = df_resultados["Duration"], df_resultados["TIR USD (%)"]
            plt.scatter(x, y, color=C_CYAN, s=200, edgecolor=C_VERDE_OSC, linewidth=1.5, zorder=5)

            if len(x) > 2:
                z = np.polyfit(x, y, 2)
                p = np.poly1d(z)
                x_trend = np.linspace(min(x), max(x), 100)
                plt.plot(x_trend, p(x_trend), color=C_LIMA, linestyle='--', linewidth=3, zorder=4, alpha=0.9)

            for i, row in df_resultados.iterrows():
                plt.annotate(row["Ticker"], (row["Duration"], row["TIR USD (%)"]), textcoords="offset points", xytext=(0,15), ha='center', fontsize=11, fontweight='bold', color=C_GRIS_OSC, bbox=dict(facecolor=C_CREMA, edgecolor='none', alpha=0.8, pad=1))
